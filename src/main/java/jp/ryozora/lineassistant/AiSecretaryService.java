...
public class AiSecretaryService {
    ...
    public String handle(String userId, String raw) {
        ...
        if (text.contains("今週忙しい") || text.contains("今週の予定")
                || text.contains("今週何ある") || text.contains("今週なにある")) {
            return weeklySummary(userId);
        }
        if (text.contains("空いてる日") || text.contains("暇な日") || text.contains("予定ない日")) {
            return freeDays(userId);
        }
        if (text.contains("買い物") || text.contains("買うもの") || text.contains("買い忘れ")) {
            return shoppingSummary(userId);
        }
        if (text.contains("忘れてない") || text.contains("忘れ物") || text.contains("今日のまとめ")) {
    ...
    private String weeklySummary(String userId) {
        ...
        List<Schedule> schedules = jdbc.query("""
                ...
                userId, Timestamp.from(today.atStartOfDay(TOKYO).toInstant()),
        ...
        for (int i = 0; i < 7; i++) {
            ...
            out.append(date.format(DateTimeFormatter.ofPattern("M/d")))
                    .append("（").append(date.getDayOfWeek().getDisplayName(TextStyle.SHORT, Locale.JAPANESE))
                    .append("）　").append(count).append("件\n");
        }
        long busiest = schedules.stream().map(s -> s.at().toLocalDate())
                .distinct().mapToLong(d -> schedules.stream().filter(s -> s.at().toLocalDate().equals(d)).count())
                .max().orElse(0);
    ...
    private String freeDays(String userId) {
        ...
        List<LocalDate> busy = jdbc.query("""
                ...
                where line_user_id=? and starts_at>=? and starts_at<?
                """, (rs, i) -> rs.getTimestamp("starts_at").toInstant().atZone(TOKYO).toLocalDate(),
                userId, Timestamp.from(today.atStartOfDay(TOKYO).toInstant()),
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (!busy.contains(date)) {
                out.append("・").append(date.format(DateTimeFormatter.ofPattern("M/d")))
                        .append("（").append(date.getDayOfWeek().getDisplayName(TextStyle.SHORT, Locale.JAPANESE))
                        .append("）\n");
                count++;
            }
    ...
    private String fullBriefing(String userId) {
        ...
        try {
            String weatherText = weather.handle(userId, "今日傘いる？");
            out.append("\n\n").append(weatherText);
